...
const mayBooks = booksByMonth['2026-05'] || [];
const juneBooks = booksByMonth['2026-06'] || [];
...
function App() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-brand-light dark:!bg-slate-900 dark:md:!bg-brand-light font-sans text-brand-dark dark:!text-gray-100 dark:md:!text-brand-dark overflow-hidden">
      {/* Desktop Sidebar */}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden pb-16 md:pb-0 relative w-full">
        {/* Top Bar */}
        <header className="px-4 md:px-8 py-4 md:py-6 flex flex-col md:flex-row justify-between items-center bg-brand-light/90 md:bg-transparent dark:!bg-slate-900 dark:md:!bg-transparent backdrop-blur-md sticky top-0 md:relative z-20 gap-4 md:gap-0">
          <div className="relative w-full md:w-96">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-secondary dark:!text-gray-300 dark:md:!text-brand-secondary" size={20} />
            <input
              type="text"
              placeholder="Buscar Livros e Autores"
              className="w-full bg-white pl-12 pr-4 py-3 rounded-2xl border-none outline-none shadow-sm focus:ring-2 focus:ring-brand-primary/20 transition-all text-sm placeholder-brand-secondary/70 dark:!bg-slate-800 dark:md:!bg-white dark:!text-white dark:md:!text-brand-dark dark:!placeholder-gray-400 dark:md:!placeholder-brand-secondary/70"
            />
          </div>
          ...
          <div className="flex items-center justify-between w-full md:w-auto gap-4 md:gap-6">
            <div className="flex gap-2 md:gap-4">
              <button
                onClick={toggleTheme}
                className="relative bg-white dark:!bg-slate-800 dark:md:!bg-white p-3 rounded-2xl shadow-sm text-brand-secondary hover:text-brand-primary dark:!text-gray-300 dark:md:!text-brand-secondary transition-colors flex-shrink-0"
              ...
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full border-2 border-white dark:!border-slate-800 dark:md:!border-white flex items-center justify-center text-[10px] text-white font-bold shadow-sm">
        ...
        <div id="main-scroll-container" className="flex-1 overflow-y-auto px-4 md:px-8 pb-12 block">
          ...
          <section className="mb-10 w-full overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-4 sm:mb-6 gap-2 sm:gap-0">
              <h2 className="text-xl sm:text-2xl font-serif font-bold text-brand-dark truncate pr-4">Livro do Mês de Maio</h2>
              <button className="flex items-center w-fit text-sm font-medium text-brand-secondary hover:text-brand-primary transition-colors gap-1 group whitespace-nowrap">
                Ver Todos <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
          ...
          <section className="mb-10 w-full overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-4 sm:mb-6 gap-2 sm:gap-0">
              <h2 className="text-xl sm:text-2xl font-serif font-bold text-brand-dark truncate pr-4">Livro do Mês de Junho</h2>
              <button className="flex items-center w-fit text-sm font-medium text-brand-secondary hover:text-brand-primary transition-colors gap-1 group whitespace-nowrap">
                Ver Todos <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)] pt-1 dark:!bg-slate-900 dark:!border-slate-800">
        <div className="flex justify-around items-center p-2">
          <button
            onClick={() => setActiveTab('home')}
            className={`flex flex-col items-center justify-center w-16 h-12 rounded-xl transition-colors ${activeTab === 'home' || activeTab === '' ? 'text-brand-primary dark:!text-brand-primary bg-brand-primary/5 dark:!bg-brand-primary/20' : 'text-brand-secondary active:bg-gray-50 dark:!text-gray-400 dark:active:!bg-slate-800'}`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center w-16 h-12 rounded-xl transition-colors ${isReviewsOpen ? 'text-brand-primary dark:!text-brand-primary bg-brand-primary/5 dark:!bg-brand-primary/20' : 'text-brand-secondary active:bg-gray-50 dark:!text-gray-400 dark:active:!bg-slate-800'}`}
          >
